...
void sigstatusbar(const Arg *arg) {
  ...
  if (!statussig)
    return;
  // sv.sival_int = arg->i; // original
  sv.sival_int = arg->i | (statussig << 8);
  // sv.sival_int = arg->i | ((SIGRTMIN + statussig) << 3); // | ((SIGRTMIN +
  // statussig) << 3) this is the change in this line
  if ((statuspid = getstatusbarpid()) <= 0)
    return;

  // sigqueue(statuspid, SIGRTMIN + statussig, sv); // original

  sigqueue(statuspid, SIGUSR1, sv); // SIGUSR1 is the change in this line
}
